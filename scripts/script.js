"use strict";
    ...
    productArray.forEach(
        item => (
            item.price.newUan ? item.price.newUan = 
                item.price.newUan.replace(/[\s\n]/, "") 
                : 
                item.price = item.price.replace(/[\s\n]/, "")
            )
    );

    let sortedProductArray = productArray.sort((product1, product2) => {

        let product1price = product1.price.newUan ? 
        +product1.price.newUan.slice(0, -3) 
        : 
        +product1.price.slice(0, -3);

        let product2price = product2.price.newUan ? 
        +product2.price.newUan.slice(0, -3) 
        : 
        +product2.price.slice(0, -3);
    ...
    sortedProductArray.forEach(product => {
        document.body.insertAdjacentHTML("beforeEnd", generateHtmlProductBlock(product));
        }
    );

    function generateHtmlProductBlock(product) {
        let price;
        if (product.price.newUan) {
            price = `
                ${generateHtmlTag("newPrice", "span", product.price.newUan)}  
                ${generateHtmlTag("oldPrice", "span", product.price.oldUan)}
            `;
        } else {
            price = `${generateHtmlTag("price", "span", product.price)}`;
        }
        ...
        let productBlock = `
            <div class="product">
                ...
                <p>Price: ${price}</p>
                <p>Reviews: ${product.ratingRevievs}</p>
            </div>
        `;
    ...
    function generateHtmlTag(tagClass, tagType, value) {
        if (!tagType && !tagClass) 
            return false;
        
        if (tagClass.indexOf(".") !== -1) 
            tagClass = tagClass.replace(/\./g, "");

        return `
        <${tagType} class=${tagClass}>
          ${value}
        </${tagType}>
        `;
    }